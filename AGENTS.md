...
- **Language / Runtime**: TypeScript 5.7, Node
- **Framework**: Next.js 14.2 (App Router), React 18, Tailwind 3.4
- **Key dependencies**: `@clerk/nextjs` (auth), `@sanity/client` (catalogue), `@neondatabase/serverless` + `drizzle-orm` (Postgres), `framer-motion`, `zod` + `react-hook-form`
- **Package manager**: npm

## Build approach

**Tracer Bullet**: prove one real thread through every layer, then thicken
it one strand at a time. Workflow tier **GA** (card payments, personal data
under Botswana's Data Protection Act 2018, a border crossing). The feature
list, phases and next steps live in [docs/scope/index.md](docs/scope/index.md).

## Commands

```bash
npm install
npm run dev          # dev server (stop it before a build)
npm run build
npm run lint
...
(`Tshego/afrideal`) is the live one and the one pushed from since 2026-09-22.
`Tshego/v2/afrideal/afrideal` is a stale sibling (it stops at `ad91cc4`,
2026-09-21) — do not work there.
...
- The Studio is moving into this repo as `studio/` (untracked as of
  2026-09-23). That copy is still partial: it lacks the `product`,
  `category`, `brand` and `runner` schemas, so `Tshego/studio/` stays the
  working Studio until the move is finished.
