...
export default async function LandingPage() {
  ...
  return (
    <>
      {/* ── The offer, priced ──────────────────────────────────────────── */}
      {/*
        The hero is image-led: a working salon, which is the room this catalogue
        is actually stocked for. The photograph runs full-bleed and the ink scrim
        is graded left-to-right rather than laid flat over the whole frame, so
        the text column sits on near-solid ink at AA while the right-hand third
        stays photograph. The ladder then lands on top as a light panel - the
        instrument set down on the scene it prices.
      */}
      <section className="relative isolate overflow-hidden bg-ink">
        <Image
          src="/unsplash/assets/hero.jpg"
          alt=""
          fill
          priority
          sizes="100vw"
        ...
        <div
          aria-hidden="true"
          className="absolute inset-0 -z-10 bg-gradient-to-r from-ink via-ink/90 to-ink/45"
        />
        <div
          aria-hidden="true"
          className="absolute inset-0 -z-10 bg-gradient-to-t from-ink/80 via-transparent to-ink/40"
        />

        <div className="mx-auto grid max-w-market gap-12 px-6 pb-16 pt-28 sm:pt-32 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:items-center lg:gap-20 lg:pb-24 lg:pt-40">
          {/*
          ...
          */}
          <div className="min-w-0">
            <h1 className="font-display text-[32px] font-bold leading-[1.06] tracking-[-0.035em] text-white sm:text-[52px] lg:text-[58px]">
              Compare the price
              <br />
              <span className="text-gold-light">before</span> you commit.
            </h1>

            <p className="measure mt-6 text-[16px] leading-8 text-white/70">
            ...
            <div className="mt-9 flex flex-wrap items-center gap-x-6 gap-y-3">
              <Link
                ...
                className="text-[14.5px] font-medium text-white underline decoration-gold decoration-2 underline-offset-[6px] transition-colors duration-300 hover:text-gold-light"
              >
                Shop hair, weaves and extensions
              </Link>
              <Link
                href="#how-it-works"
                className="text-[14.5px] text-white/65 underline-offset-4 transition-colors duration-300 hover:text-white hover:underline"
              >
                How AfriDeal works
              </Link>
            </div>
            ...
            {/*
                ...
                fact.
              */}
            <p className="mt-10 border-t border-white/15 pt-6 text-[13px] leading-6 text-white/55">
          ...
          */}
          {featured && doors.length > 0 && (
            <Reveal delay={0.1} className="min-w-0">
              {/*
                A light ring and a deeper shadow, because the panel is now dark
                on a dark photograph rather than dark on the light page. The
                internals are untouched: the edge is what separates it, so the
                ladder still reads as an instrument set down on the scene
                instead of a hole cut in it.
              */}
              <div className="grain relative overflow-hidden rounded-xl bg-ink p-2 shadow-[0_24px_60px_-16px_rgba(0,0,0,0.7)] ring-1 ring-white/15">
                <div
                  aria-hidden="true"
